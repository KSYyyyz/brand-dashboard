...
function generateUsers(count = 500) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const firstOrderTime = orderCount > 0 ? new Date(createdAt.getTime() + random(1, 30) * 24 * 60 * 60 * 1000) : null
    const lastOrderTime = orderCount > 1 ? new Date(firstOrderTime.getTime() + random(1, 180) * 24 * 60 * 60 * 1000) : firstOrderTime
...
function generateOrders(users, count = 1500) {
  const orders = []
  const now = new Date()
  let orderId = 1
...
function generateStoreSales(stores, days = 30) {
  const sales = []
  let id = 1
  const now = new Date()
...
function generateAdsData(days = 30) {
  const data = []
...
function generateContentData(days = 30) {
  const data = []
...
async function uploadData() {
  ...
  console.log(`生成完成: ${users.length}用户, ${orders.length}订单, ${stores.length}门店`)

  // 清空现有数据
  console.log('清空现有数据...')
  await supabase.from('content_data').delete().neq('id', 0)
  await supabase.from('ads_data').delete().neq('id', 0)
  await supabase.from('store_sales').delete().neq('id', 0)
  ...
  if (error) console.error('用户数据错误:', error)
  else console.log('用户数据上传成功')

  console.log('上传订单数据...')
  ;({ error } = await supabase.from('orders').insert(orders))
  if (error) console.error('订单数据错误:', error)
  else console.log('订单数据上传成功')

  console.log('上传门店数据...')
  ;({ error } = await supabase.from('stores').insert(stores))
  if (error) console.error('门店数据错误:', error)
  else console.log('门店数据上传成功')

  console.log('上传销售数据...')
  ;({ error } = await supabase.from('store_sales').insert(storeSales))
  if (error) console.error('销售数据错误:', error)
  else console.log('销售数据上传成功')

  console.log('上传投流数据...')
  ;({ error } = await supabase.from('ads_data').insert(adsData))
  if (error) console.error('投流数据错误:', error)
  else console.log('投流数据上传成功')

  console.log('上传内容数据...')
  ;({ error } = await supabase.from('content_data').insert(contentData))
  if (error) console.error('内容数据错误:', error)
  else console.log('内容数据上传成功')
